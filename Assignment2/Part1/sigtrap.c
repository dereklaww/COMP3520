...
usage:
sigtrap [n]
[n] is time for process to exist - default 60 seconds
...
1.1: Altered default sleep duration
...
Australia
2. Dr Ian G Graham
Copyright of this code and associated material is vested in the original
contributors.
This code is NOT in the Public Domain. Unauthorized posting of this code or
derivatives thereof is not permitted.
...
int main(int argc, char *argv[])
{
    ...
    for (i = 0; i < cycle;) { // tick
        ...
        if (rc == 0 || (stoptick-starttick) > clktck/2) fprintf(output,"%s%7d; tick %d" BLACK NORMAL "\n", colour, (int) pid, ++i);
        ...
        if (signal_SIGTSTP) {
            ...
            fflush(output);
            sigemptyset (&mask); // unblock SIGSTP if necessary (BSD/OSX)
