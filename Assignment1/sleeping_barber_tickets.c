...
pthread_cond_t customer_arrived, barber_ready_cond, customer_ready, barber_done, customer_signalcond, shop_closed_cond;
...
void *assistant_routine(void *arg) {
    ...
    while (1) {
        ...
        if (!barber_busy && (no_leaving == no_of_customers)) {
